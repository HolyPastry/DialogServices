    ...
    public class DialogTagProcessor : TagProcessor
    {
        ...
        public override List<string> ProcessTags(ThespianData character, List<string> tags)
        {
            ...
            if (Parse(ref newTags, "DelayBefore", out List<string> delays))
                Dialogs.Manager().AddDelay(EnumDelayType.BeforeLine, ToFloat(delays[0]));

            if (Parse(ref newTags, "DelayAfter", out List<string> delays2))
                Dialogs.Manager().AddDelay(EnumDelayType.AfterLine, ToFloat(delays2[0]));

            if (Parse(ref newTags, "Overlap", out List<string> overlaps))
                Dialogs.Manager().AddDelay(EnumDelayType.OverlapLine, ToFloat(overlaps[0]));
